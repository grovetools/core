...
.PHONY: all build test clean fmt fmt-check vet lint run check dev build-all schema help setup
...
build: schema
	...
	@$(GO_CROSS_ENV) go build $(LDFLAGS) -o $(BIN_DIR)/$(BINARY_NAME) ./cmd/core
...
dev:
	...
	@go build -race $(LDFLAGS) -o $(BIN_DIR)/$(BINARY_NAME) ./cmd/core
...
build-all:
	...
	@for platform in $(PLATFORMS); do \
		...
		GOOS=$$os GOARCH=$$arch go build $(LDFLAGS) -o $(DIST_DIR)/$${output_name} ./cmd/core; \
	done

# --- E2E Testing ---
# Run E2E tests. Depends on the main 'core' binary.
...
help:
	...
	@echo "  make test-e2e ARGS=...- Run E2E tests (e.g., ARGS=\"-i core-config-layering\")"
