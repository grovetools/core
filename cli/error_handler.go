...
func (h *ErrorHandler) Handle(err error) error {
	...
	case errors.ErrCodeConfigNotFound:
		fmt.Fprintf(os.Stderr, "Error: Configuration not found. Run 'grove init' to create a new configuration.\n")
		return err

	case errors.ErrCodeServiceNotFound:
		if groveErr, ok := err.(*errors.GroveError); ok {
			fmt.Fprintf(os.Stderr, "Error: Service '%s' not found in grove.yml\n", groveErr.Details["service"])
	...
	case errors.ErrCodeCommandNotFound:
		fmt.Fprintf(os.Stderr, "Error: Required command not found. Make sure Docker and docker-compose are installed.\n")
		return err

	case errors.ErrCodeContainerTimeout:
		if groveErr, ok := err.(*errors.GroveError); ok {
			fmt.Fprintf(os.Stderr, "Error: Service '%s' failed to start within %s\n",
