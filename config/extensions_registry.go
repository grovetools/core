...
var knownExtensions = map[string]ExtensionInfo{
	...
	"forge":         {Key: "forge", Repo: "core", Description: "Self-hosted forge client contract: instance (url, remote_name, token_command, provider) and the daemon poller ([forge.poll]); see core/config/forge.go. The provisioning sub-blocks ([forge.infra], [forge.services], [forge.backup], [forge.wireguard]) are owned by the grove-plugin-forge-gcp recipe and decode from this same namespace"},
