...
const (
	// PluginsDirName is the drop-in directory's name under the grove config
	// dir.
	PluginsDirName = "plugins"
	// FragmentGlob is what core/config matches inside PluginsDirName. It is a
	// constant here, and core/config globs with FragmentPattern rather than
	// spelling the pattern itself, because the two have to agree: the installer
	// writes files this glob must find, and the lockfile sits in the same
	// directory and must NOT be found (see the note above). One definition is
	// what keeps that invariant checkable in one place.
	FragmentGlob = "*.toml"
)

// FragmentPattern is the glob that finds every plugin manifest fragment under a
// grove config directory.
//
// It takes the directory rather than resolving it, because core/config derives
// its global directory from wherever the global config file was actually found,
// which is not always paths.ConfigDir(). ConfigPluginsDir below is the
// installer's view of the same directory.
func FragmentPattern(configDir string) string {
	return filepath.Join(configDir, PluginsDirName, FragmentGlob)
}
...
func ConfigPluginsDir() (string, error) {
	...
	return filepath.Join(dir, PluginsDirName), nil
}
