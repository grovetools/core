...
//   ~/notebooks/grovetools/notespaces/<repo>/). Flow agents read briefings/
...
//     (notespaces/<repo>), where .artifacts/plans/concepts live. core/pkg/
...
//     ~/notebooks/grovetools/notespaces/nb/, exactly the union member we want.
...
//     dirs share parent notespaces/<repo>, so filepath.Dir(GetNotesDir(node,
...
// the parent of the per-note-type directories (notespaces/<repo>), where
...
func notebookRootForNode(locator *NotebookLocator, node *WorkspaceNode) string {
	...
	// GetNotesDir(node,"inbox") = <root>/notespaces/<repo>/inbox (centralized)
	// or <repo>/.notebook/notes/inbox (local). The paired notebook root is the
	// grandparent for centralized (drop notes/inbox -> notespaces/<repo>) — but
	...
	// root:
	//   centralized: <root>/notespaces/<repo>/inbox -> Dir -> .../<repo>  (the
	//                notes dir is flat: GetNotesDir returns notespaces/<repo>/inbox,
	//                so a single Dir already gives notespaces/<repo>).
	// Empirically (verified via `nb context --json`) the per-type dirs are
	// siblings directly under notespaces/<repo>, so ONE Dir is correct for
