...
func TransformToWorkspaceNodes(result *DiscoveryResult, cfg *config.Config) []*WorkspaceNode {
	...
	for _, proj := range result.Projects {
		...
		// Is this project an ecosystem worktree? Prefer the explicit
		// provenance discovery records (layout-independent); fall back to
		// the legacy path shape for results without provenance.
		isEcoWorktree := proj.WorktreeOwnerPath != ""
		if !isEcoWorktree && proj.ParentEcosystemPath != "" {
		...
		if isEcoWorktree {
			...
			// Parent paths point at the ORIGINAL checkout (node identity
			// contract) — never the XDG container.
			ownerPath := proj.WorktreeOwnerPath
			if ownerPath == "" {
				ownerPath = proj.ParentEcosystemPath
			}
			nodes = append(nodes, &WorkspaceNode{
		...
		// Is this project inside an ecosystem worktree? Provenance on the
		// containing project decides (layout-independent); path shape is the
		// fallback for results without provenance.
		insideEcoWorktree := false
		if isSubProject {
			if parent, ok := projectMap[proj.ParentEcosystemPath]; ok && parent.WorktreeOwnerPath != "" {
				insideEcoWorktree = true
			} else if IsWorktreePath(proj.Path) {
				insideEcoWorktree = true
			}
		}
		...
		for _, ws := range proj.Workspaces {
			if ws.Type == WorkspaceTypeWorktree {
				...
				if isSubProject {
					// A worktree of a sub-project that itself lives inside an
					// ecosystem worktree is linked development.
					if insideEcoWorktree {
